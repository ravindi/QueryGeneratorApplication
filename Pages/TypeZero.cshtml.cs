...
{
    public class TypeZeroModel : PageModel
    {
        ...
        public void OnGet()
        {
            if (!string.IsNullOrEmpty(Data))
            {
                ...
                Parallel.ForEach(DeserializedData, options, item =>
                {
                    ...
                    string queries = GenerateQueries(entity, state, perception);
                ...
                if (entity == "Car" && (obj == "Bicycle" || obj == "Car"))
                {
                    if (!string.IsNullOrEmpty(relation))
                    {
                       ...
                       || relation.Contains("leftOf") || relation.Contains("infrontOf")))
                        ...
                        if (entity == "Car" && relation.Contains("open") && perception == "ThirdPerson")
                        {
                            string query2 = @$"
                                prefix mv:http://mobivoc.org
                                pull (car.vin)
                ...
                if (perception == "ThirdPerson")
                {
                    foreach (string operandValue in operand)
                    {
                        string query2 = @$"
                                ...
                                pull (bicycle.id)
                                define
                                entity bicycle is FROM mv:Bicycle where bicycle.proximityToObstacle {operandValue} {GenerateRandomNumericValue(15, 50)}";
                        queries.Add(query2);
        ...
        public string GenerateQueries(string entity, string state, string perception)
            {

                var personId = GenerateRandomId("Person");

                string[] bicycleRelatedAttributes = { "speed", "conditionOfBrakes", "wearingOfTyres",
                    "roadCondition","roadSurface", "trafficCondition", "proximityToVehicle"};
                Dictionary<string, object> bicycleRelatedAttributeValues = new Dictionary<string, object>
                {
                    ...
                    { "roadSurface", GenerateRandomRoadSurfaceEnum() },
                ...
                if (entity == "Bicycle" && (state == "Riding" || state == "KeepingLane" || state == "Stopping"))
                {
                    ...
                    if (perception == "ThirdPerson")
                    {
                        foreach (string operandValue in operand)
                        {
                            ...
                            string query2 = @$"
                                ...
                                pull (person.*)
                                define
                                entity person is FROM mv:Person where person.{field} {operandValue} {formattedValue};";
                            queries.Add(query2);
                            ...
                            string query4 = @$"
                                ...
                                define entity person is from mv:Person where person.{field} {operandValue} {formattedValue};";
                            queries.Add(query4);
                        }
            ...
            else if (value is int || value is double || value is float)
            {
                ...
                foreach (string operandValue in operand)
                {
                    ...
                    @$"
                            ...
                            pull (bicycle.*)
                            define
                            entity bicycle is FROM mv:Bicycle where bicycle.{field} {operandValue} {formattedValue};";
                    queries.Add(query3);
                }
